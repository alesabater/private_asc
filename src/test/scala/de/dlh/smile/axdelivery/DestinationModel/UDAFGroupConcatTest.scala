...
//	
    ...
    // add column for each day with the Sale value if days match:
    val withDayColumns = distinctValues.foldLeft(df) { 
      case (data, dvalue) => data.selectExpr("*", s"IF(mdlrank = '$dvalue', Month, '') AS $dvalue")
    }
     ...
     .drop("Month")
     .groupBy("Customer")
     .agg(UDAFGroupConcat(col("Fri")).alias("Fri"),
         UDAFGroupConcat(col("Mon")).alias("Mon")
         )
...
//        MovingAverage.getMovingAverage(df)
//        )       
//    dfTmp.show
//	
//		// get distinct days from data (this assumes there are not too many of them):
...
//	
//		// get distinct days from data (this assumes there are not too many of them):
//		val distinctValues: Array[String] = dfTmp.select("mdlrank")
//    .distinct()
//    .collect()
//    .map(_.getAs[String]("mdlrank"))
//    
//    // add column for each day with the Sale value if days match:
//    val withDayColumns = distinctValues.foldLeft(dfTmp) { 
//      case (data, dvalue) => data.selectExpr("*", s"IF(mdlrank = '$dvalue', BFD, '') AS $dvalue")
